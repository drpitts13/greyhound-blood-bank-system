...
| `INVLOC-CREATE-PERM` | Caller has `admin.config.edit` when creating an inventory location | HardStop when a permission evaluator is present and the privilege is missing |
