...
- The pending test worklist (`GET /api/test-worklist/pending`, `/test-worklist`) shows current ABO/Rh, antibody history (including currently undetectable), specimen expiration, electronic-XM eligibility, and an open antibody-identification workup on each row so staff do not leave the bench list to check dating, history, or eXM holds. Entry is still blocked when the specimen is missing, not Accepted, or expired. The Patient link opens `/patients/{id}?tab=tests`. ABID opens `/patients/{id}/antibody-id/{workupId}`. The standalone `/compatibility` page identifies the recipient by MRN/name search and the specimen by accession, not by internal numeric ids.
