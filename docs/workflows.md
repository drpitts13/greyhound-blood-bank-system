...
- Documenting a transfusion requires two independent patient identity tokens that match the issued patient (`ISS-IDENTITY`). A checkbox is not positive identification. Electronic dual-ID (`TX-DUAL-ID`) is complete only when those tokens match and an ISBT bedside unit scan verifies. Legacy units keep the prior scan policy. Second-verifier policy remains optional in Facility Policy (OCD-008).
