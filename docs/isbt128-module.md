...
- Lookup tables: `IsbtProductCodes` is seeded with a commonly published US supplier subset of Product Description Codes (`US-PUBLIC-SUBSET-PENDING-ICCBBA` in `UsSupplierProductCodeSeed`); replace via licensed import before clinical use — do not invent codes
- Admin UI: **Administration → ISBT Product Codes** (`/admin/isbt-product-codes`) lists seeded codes (`admin.config.view`) and, with `admin.config.edit`, imports a licensee extract: JSON paste, drop-folder CSV/TSV/JSON (`Iccbba:ExtractDirectory`, default `testdata/isbt128/extracts`), or file upload. Access/Excel databases are rejected (export first). Empty folder keeps placeholders. License acknowledgment and a non-placeholder `StandardVersion` are required.
...
1. Official ICCBBA product and ABO/RhD tables are licensed IP — shipped product rows are a **US public subset** (`IsPlaceholder=true`) pending facility ICCBBA validation; ABO/RhD demo codes remain placeholders. Cycle 7 added JSON import; Cycle 14 adds CSV/TSV/JSON extract import if files are available. A licensed extract has not been loaded (OCD-004).
