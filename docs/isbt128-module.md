...
- Lookup tables: `IsbtProductCodes` is seeded with a commonly published US supplier subset of Product Description Codes (`US-PUBLIC-SUBSET-PENDING-ICCBBA` in `UsSupplierProductCodeSeed`); replace via licensed import (`POST /api/admin/isbt-product-codes/import-licensed`) before clinical use — do not invent codes
- Admin UI: **Administration → ISBT Product Codes** (`/admin/isbt-product-codes`) lists seeded codes (`admin.config.view`) and, with `admin.config.edit`, accepts a licensee JSON extract plus version and license acknowledgment
...
1. Official ICCBBA product and ABO/RhD tables are licensed IP — shipped product rows are a **US public subset** (`IsPlaceholder=true`) pending facility ICCBBA validation; ABO/RhD demo codes remain placeholders. Cycle 7 adds an import path; a licensed extract has not been loaded (OCD-004).
