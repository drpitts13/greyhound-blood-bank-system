...
public class TestServiceBillingAuthorizationTests : IClassFixture<SqliteContextFactory>
{
    ...
    public async Task CreateAndUpdate_WriteConfigure()
    {
        ...
        var created = await svc.CreateAsync(
            new SaveTestServiceBillingRequest(code.Id, null, BillingTriggerType.TestVerified, test));
        ...
        Assert.True(await c.AuditEvents.AnyAsync(a =>
            a.EntityType == nameof(TestServiceBilling)
            && a.EntityId == created.Value!.Id
            && a.EventType == AuditEventType.Configure));

        var updated = await svc.UpdateAsync(
            created.Value!.Id,
            new SaveTestServiceBillingRequest(code.Id, "Verified panel", BillingTriggerType.TestVerified, test));
        ...
        var events = await c.AuditEvents
            .Where(a => a.EntityType == nameof(TestServiceBilling) && a.EntityId == created.Value.Id)
            .ToListAsync();
