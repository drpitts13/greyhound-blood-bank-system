...
- Phase 7 — Billing: complete (event-driven charge capture isolated from clinical decisions: data-driven `ChargeCodes` + `ChargeRules` plus `TestServiceBillings` / `ProductBillings` catalogs map triggers — `TestVerified`, `UnitIssued` — to charges; a `BillingService` translates committed clinical actions into `BillingEvents` with a deterministic, unique `DedupeKey` so a repeated trigger can never double-charge; capture is wired into the verify-result and issue endpoints so a blocked/failed action never bills; a review queue with `Pending -> Reviewed -> Exported` flow plus reason-gated, audited `Cancelled`; capture also queues a standard outbound DFT^P03 with billing code and no price).
- Phase ISBT 128 — Blood-component identification & workflow: complete (normalization pipeline, scan session, expanded inventory state machine, versioned lookup/rules tables, identity correction, docs in `docs/isbt128-module.md`). Not a certification claim.
...
- Phase 9 — Product modification: complete (admin-configured `ModificationRules` catalog mapping source product + modification type — divide/pool/irradiate/thaw/volume-reduction/leukoreduction — to a target product with an expiration offset code such as `24H`/`5D`; a unified `BloodProductModificationService` execution path that guards eligibility, retires source unit(s) to the terminal `Modified` status, creates result unit(s) in `Quarantine` with capped expiration (`min(PerformedUtc + offset, earliest source expiry)`), links everything via `UnitModification`/`UnitModificationUnit`, and audits every modification; a clinical UI to perform modifications and an admin UI to manage the rules catalog).
...
- Product modification: `GET /api/inventory/units/{id}/eligible-modifications`, `GET /api/inventory/units/{id}/modifications` (history), `POST /api/inventory/units/{id}/modifications/divide`, `POST /api/inventory/modifications/pool`, `POST /api/inventory/units/{id}/modifications/apply` (irradiate/thaw/volume-reduce/leukoreduce)
- Admin: `GET/POST/PUT /api/admin/modification-rules`, `POST /api/admin/modification-rules/{id}/activate`, `POST /api/admin/modification-rules/{id}/deactivate` (plus analogous CRUD for `products`, `tests`, `blood-attributes`, `specimen-types`, `subtests`, `test-groupers`, `reflex-rules`, `isbt-product-codes`, `providers`, `exceptions`, `locations`, `hl7/endpoints`, `users`, `roles`)
